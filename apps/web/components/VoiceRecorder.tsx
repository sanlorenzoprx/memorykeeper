...
export default function VoiceRecorder({ photoId }: { photoId: string }) {
  ...
  const handleUploadAndTranscribe = async (audioBlob: Blob) => {
    ...
    try {
      ...
      const { uploadUrl, key } = await apiPost('/api/audio/uploads', { filename }, token || undefined);
      ...
      await apiPost(`/api/photos/${photoId}/transcribe`, { r2Key: key }, token || undefined);

      // 4. Trigger gamification action for captioning
      await apiPost('/api/gamification/actions/caption', {}, token || undefined);
